...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $role = 3; //1 = member, 2 = trainer, 3 = dietician
    ...
    if ($Rownum == 0) {
        ...
        if ($pw == $cpw) {
            ...
            if (
                !empty($fname) && !empty($lname) && !empty($nic) && !empty($gender) && !empty($num) && !empty($dob) && !empty($username) && !empty($pw) && !empty($cpw) && !empty($language)
...
<head>
    ...
    <title>Add Dietician | Admin</title>
...
<body>
    ...
    <form class=addTrainerForm action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>' method="POST" onsubmit="return validateForm()">
        ...
        <div class="form-row">
            ...
            <div class="form-group" style="margin-right:50px;margin-left: 220px;">
                ...
                <br>
                <input type="password" id="pass1" name="pass1" maxlength="15">
            </div>
