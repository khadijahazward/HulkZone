...
$query = "SELECT * FROM supplement WHERE employeeID = $employeeID AND memberID = $memberID";
$result = mysqli_query($conn, $query);
if (mysqli_num_rows($result1) > 0) {
    $row1 = mysqli_fetch_assoc($result);
} else {
    echo '<script> window.alert("Error of receiving supplement data!");</script>';
}
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($result1)  == 1) {
        if (isset($_POST['submit'])) {
            ...
            $query1 =   "UPDATE supplement 
                SET supplementName = '$supplement',
                suplementType = '$supplementType',
                WHERE employeeID = $employeeID
                AND memberID = $memberID";

            $supplement1 = mysqli_query($conn, $query1);

            if ($supplement1) {
                echo "<script> alert('Updated Successfully!'); window.location='dietPlan.php'; </script>";
            } else {
                echo "<script> alert('Error of updating supplement details');</script>";
            }
        }
    }
}
